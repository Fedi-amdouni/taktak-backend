...
final class UnoState {
    ...
    void leave(String id) { if (id != null) { players.remove(id); hands.remove(id); } if (players.isEmpty()) { started = false; turnId = null; topCard = null; winner = null; } else if (Objects.equals(turnId, id)) { turnId = players.keySet().iterator().next(); } }
